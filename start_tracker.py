...
def create_group(req):
    ...
    if req.method == "OPTIONS": return ("HTTP/1.1 204 No Content\r\n" + f"{cors_headers}\r\n").encode("utf-8")

    try:
        data = json.loads(req.body)
        group_name = data.get("group_name")
        username = data.get("username") or (req.cookies.get("username") if req.cookies else None)
        
        if not group_name or not username:
            body = json.dumps({"status": "error", "message": "Thiếu tên nhóm hoặc chưa đăng nhập"})
            return (f"HTTP/1.1 400 Bad Request\r\nContent-Type: application/json\r\n{cors_headers}Content-Length: {len(body.encode('utf-8'))}\r\n\r\n{body}").encode("utf-8")
            
        if group_name in group_list:
            body = json.dumps({"status": "error", "message": f"Nhóm '{group_name}' đã tồn tại!"})
            return (f"HTTP/1.1 400 Bad Request\r\nContent-Type: application/json\r\n{cors_headers}Content-Length: {len(body.encode('utf-8'))}\r\n\r\n{body}").encode("utf-8")
        ...
        body = json.dumps({"status": "success", "message": "Tạo nhóm thành công", "group_name": group_name})
...
def add_to_group(req):
    ...
    try:
        ...
        requester = data.get("username") or (req.cookies.get("username") if req.cookies else None) # Người bấm nút
        ...
        if group_list[group_name]["owner"] != requester:
            body = json.dumps({"status": "error", "message": "Chỉ Trưởng nhóm mới có quyền thêm người!"})
        ...
        body = json.dumps({"status": "success", "message": f"Đã thêm {target_user} vào nhóm {group_name}"})
        return (f"HTTP/1.1 200 OK\r\nContent-Type: application/json\r\n{cors_headers}Content-Length: {len(body.encode('utf-8'))}\r\n\r\n{body}").encode("utf-8")
    except Exception as e:
        return Response().build_internal_error({"message": str(e)})
...
def my_groups(req):
    ...
    try:
        username = req.query_params.get("username") or (req.cookies.get("username") if req.cookies else None)
        if not username:
